...
pub(super) fn find_position_in_wrapped_spans(
    ...
    for (row, spans) in wrapped_spans.iter().enumerate() {
        ...
        if char_pos <= max_char_pos {
            ...
            // tnotes patch: a row may end with a hanging wrap space that sits at
            // `max_width`; a cursor there is shown at the start of the next row.
            if col >= max_width && row + 1 < wrapped_spans.len() {
                return Index2::new(row + 1, 0);
            }
            return Index2::new(row, col.min(max_width.saturating_sub(1)));
        }
